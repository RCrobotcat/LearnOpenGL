...
int main()
{
    ...
    // shaders
    Shader shader_gbuffer_pass("../shaders/DeferredShading/g_buffer.vs",
                               "../shaders/DeferredShading/g_buffer.fs");
    Shader shader_lighting_pass("../shaders/DeferredShading/deferred_shading.vs",
                                "../shaders/DeferredShading/deferred_shading.fs");
    Shader shader_lightbox_pass("../shaders/DeferredShading/deferred_lightbox.vs",
                                "../shaders/DeferredShading/deferred_lightbox.fs");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        shader_gbuffer_pass.use();
        shader_gbuffer_pass.setMat4("projection", projection);
        shader_gbuffer_pass.setMat4("view", view);
        for (unsigned int i = 0; i < objectPositions.size(); i++)
        {
            model = glm::mat4(1.0f);
            model = glm::translate(model, objectPositions[i]);
            model = glm::scale(model, glm::vec3(0.5f));
            shader_gbuffer_pass.setMat4("model", model);
            ourModel.Draw(shader_gbuffer_pass);
        }
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
        ...
        // --------------------------------
        shader_lightbox_pass.use();
        shader_lightbox_pass.setMat4("projection", projection);
        shader_lightbox_pass.setMat4("view", view);
        for (unsigned int i = 0; i < lightPositions.size(); i++)
        {
            model = glm::mat4(1.0f);
            model = glm::translate(model, lightPositions[i]);
            model = glm::scale(model, glm::vec3(0.125f));
            shader_lightbox_pass.setMat4("model", model);
            shader_lightbox_pass.setVec3("lightColor", lightColors[i]);
            renderCube();
        }
